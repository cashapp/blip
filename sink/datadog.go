	...
	METRICS:
		for i := range metrics { // each metric in this domain
			...
			if tsStr, ok := metrics[i].Meta["ts"]; !ok {
				...
				var err error
				msTs, err = strconv.ParseInt(tsStr, 10, 64) // ts in seconds (unix epoch), string -> int64
				if err != nil {
					blip.Debug("invalid timestamp for %s %s: %s: %s", domain, metrics[i].Name, tsStr, err)
					continue METRICS
				}
				timestamp = time.UnixMillis(msTs).Unix()
			}
...
func (s *Datadog) sendApi(ddCtx context.Context, dp []datadogV2.MetricSeries) error {
	...
	for rangeStart := 0; rangeStart < len(dp); {
		...
		payloadResponse, r, err := s.metricsApi.SubmitMetrics(ddCtx, *datadogV2.NewMetricPayload(dp[rangeStart:rangeEnd]), optParams)
		...
		if len(payloadResponse.Errors) > 0 {
			return fmt.Errorf("datadog returned error for payload: %s", strings.Join(payloadResponse.Errors, ","))
		}
